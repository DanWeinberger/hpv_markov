...
### This model is for the detected compartments ONLY
...
# Treatment
p_3_1 <- 0.5*0.9 # HSIL > normal (proportion treated*treatment success)
p_4_1 <- 0.5 # Cancer > normal (treatment success)
...
for(i in 2:t){
  # Normal
  mat1[i,1] <- (mat1[(i-1),1]) +  
    (Pop_size*p_age_in) +           # Age in
    (mat1[(i-1),2])*p_2_1 +         # Regress from LSIL
    (mat1[(i-1),3])*p_3_1 +         # Successful treatment for HSIL
    (mat1[(i-1),4])*p_4_1 -         # Successful treatment for cancer
    (mat1[(i-1),1])*p_1_2 -         # Progression to HSIL
    (mat1[(i-1),1])*p_age_out -     # Age out
    (mat1[(i-1),1])*p_die           # Die of unrelated cause
  # LSIL  
  mat1[i,2] <-   (mat1[(i-1),2]) + 
    (mat1[(i-1),1])*p_1_2 +
    (mat1[(i-1),3])*p_3_2 - 
