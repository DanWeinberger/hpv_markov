...
Pop_size_4 = (154566548-Pop_size_0-Pop_size_1-Pop_size_2-Pop_size_3)*0.5

# aging into cohort
age_in <- -log(1-0.05)/12 # multiply by pop under 18
# aging up within cohort
age_up_0 <- -log(1-1/3)/12 # proportion turning 21
age_up_1 <- -log(1-0.25)/12 # proportion turning 25
age_up_2 <- -log(1-0.2)/12 # proportion turning 30
age_up_3 <- -log(1-0.1)/12 # proportion turning 40
# Dying
die_1 <- -log(1-74/100000)/12 # proportion dying age 18-24
die_2_3 <- -log(1-164/100000)/12 # proportion dying age 25-39
die_4 <- -log(1-1500/100000)/12 # proportion dying age 40+

# undetected
norm_ulsil_0 <- -log(1-0.15)/12 # normal > undetected LSIL (18-20)
norm_ulsil_1 <- -log(1-0.08)/12 # normal > undetected LSIL (21-24)
norm_ulsil_2 <- -log(1-0.02)/12 # normal > undetected LSIL (25-29)
norm_ulsil_3 <- -log(1-0.01)/12 # normal > undetected LSIL (30-39)
norm_ulsil_4 <- 0 # normal > undetected LSIL (40+)
ulsil_norm_1 <- -log(1-0.60)/12 # undetected LSIL > normal (18-24)
ulsil_norm_2_3 <- -log(1-0.4)/12 # undetected LSIL > normal (25-39)
ulsil_uhsil_1 <- -log(1-0.14)/12 # undetected LSIL > undetected HSIL (18-24)
ulsil_uhsil_2_3 <- -log(1-0.30)/12 # undetected LSIL > undetected HSIL (25-39)
ulsil_uhsil_4 <- -log(1-0.30)/12 # undetected LSIL > undetected HSIL (40+)
uhsil_ulsil_1 <- -log(1-0.62)/12 # undetected HSIL > undetected LSIL (18-24)
uhsil_ulsil_2_3 <- -log(1-0.35)/12 # undetected HSIL > undetected LSIL (25-39)
uhsil_ulsil_4 <- -log(1-0.30)/12 # undetected HSIL > undetected LSIL (40+)
uhsil_ucan_2_3 <- -log(1-6/100000)/12 # undetected HSIL > undetected cancer (21-39)
uhsil_ucan_4 <- -log(1-12/100000)/12 # undetected HSIL > undetected cancer (40+)
...
dcan_dcandeath <- -log(1-0.35)/12 # detected cancer > detected cancer death

# treatment
dhsil_norm <- -log(1-0.9*0.9)/12 # detected HSIL > normal (% success * % treated)
dcan_norm <- -log(1-0.5)/12 # detected cancer > normal (% success (based on ~5 year survival) * % treated)

# hysterectomies
hyst_1 <- -log(1+log(0.99)/10)/12 # hysterectomy 21-29 (normalized from 10 year rate > 1 year rate)
hyst_2 <- -log(1+log(0.96)/10)/12 # hysterectomy 30-39 (normalized from 10 year rate > 1 year rate)
hyst_3 <- -log(1+log(0.70)/10)/12
...
for(i in 2:t){
  ...
  # screening
  ulsil_dlsil <- -log(1-0.83/3)/12 # no change in screening
  uhsil_dhsil <- -log(1-0.83/3)/12 # no change in screening
  ucan_dcan <- -log(1-0.83/3)/12 # no change in screening
  
  # loss to follow up
  dlsil_uhsil <- -log(1-0.17/3)/12 # no change in LTFU
  dhsil_ucan <- -log(1-0.17/3)/12 # no change in LTFU
...
for(i in 2:t){
  ...
  # vaccination
  norm_ulsil_0 <- ifelse(t.index<988*12, -log(1-0.15)/12, -log(1-0.15*0.2)/12) # starting in year 988 (2008), vaccination drops incidence by 80%
  norm_ulsil_1 <- ifelse(t.index<988*12, -log(1-0.08)/12, -log(1-0.08*0.2)/12) # starting in year 988 (2008), vaccination drops incidence by 80%
  norm_ulsil_2 <- ifelse(t.index<988*12, -log(1-0.02)/12, -log(1-0.02*0.2)/12) # starting in year 988 (2008), vaccination drops incidence by 80%
...
result_tot_noCov$Year <- (result_tot_noCov$Year.No/12)+1020
...
# Normal
p_Norm_month<-ggplot(data=result_tot_noCov, aes(x=Year)) +
  geom_line(data=result_tot_noCov, aes(y=Normal), color="red") + 
  coord_cartesian(
    xlim = c(1950,2020),
...
# LSIL
p_LSIL<-ggplot(data=result_tot_noCov, aes(x=Year)) +
  geom_line(data=result_tot_noCov, aes(y=Undet_LSIL), color="red", linetype="dashed") + 
  geom_line(data=result_tot_noCov, aes(y=Det_LSIL), colour="red") +
  geom_line(data=result_tot_noCov, aes(y=All_LSIL), color="black") +
  coord_cartesian(
    xlim = c(1000,2050),
    ylim = c(0,1500000)) +
  geom_vline(xintercept = 2008,linetype="dashed",alpha=0.3) +
...
# HSIL
p_HSIL<-ggplot(data=result_tot_noCov, aes(x=Year)) +
  geom_line(data=result_tot_noCov, aes(y=Undet_HSIL), color="red", linetype="dashed") + 
  geom_line(data=result_tot_noCov, aes(y=Det_HSIL), colour="red") +
  geom_line(data=result_tot_noCov, aes(y=All_HSIL), color="black") +
  coord_cartesian(
    xlim = c(2000,2050),
    ylim = c(0,400000)) +
  geom_vline(xintercept = 2008,linetype="dashed",alpha=0.3) +
